  ...
  render () {
    ...
    return html`
      ...
      if (isMedia) {
        ...
        if (isImage) {
          return html`<div><img style="max-width: 60%; height: auto;" src=${piece} alt="User content" /></div>`
        } else if (isVideo) {
          return html`<div><video style="max-width: 60%; height: auto;" controls src=${piece}></video></div>`
        }
      } else if (isYoutube) {
